...
class Linear(object):
    ...
    def forward(x, w, b):
        ...
        out = torch.mm(x_reshaped, w)
        bb = b.repeat(out.shape[0], 1)
        out = out + bb
        ######################################################################
        #                        END OF YOUR CODE                            #
        ######################################################################
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        w1 = torch.normal(mean=0.0, std=weight_scale, size=(input_dim,hidden_dim))
        w1 = w1.type(dtype)
        b1 = torch.zeros(hidden_dim, dtype=dtype)
        w2 = torch.normal(mean=0.0, std=weight_scale, size=(hidden_dim,num_classes))
        w2 = w2.type(dtype)
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        X_reshaped = torch.reshape(X, (X.shape[0], -1))
        linear1 = torch.mm(X_reshaped, self.params["W1"]) + self.params["b1"]
        relu1 = linear1.clone()
        relu1[(relu1 < 0)] = 0
        linear2 = torch.mm(relu1, self.params["W2"]) + self.params["b2"]
        scores = linear2
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        L2 = (self.params["W1"]**2).sum().item()+(self.params["W2"]**2).sum().item()
        loss += self.reg * L2
        
        # p=xw1+b1 q=relu(p) f=qw2+b2
        dq = torch.mm(df, self.params["W2"].t())
        dw2 = torch.mm(relu1.t(), df) + self.reg*2*self.params["W2"]
        db2 = df.sum(dim = 0)
        ...
        dx = torch.mm(dp, self.params["W1"].t()).reshape(X.shape)
        dw1 = torch.mm(X_reshaped.t(), dp) + self.reg*2*self.params["W1"]
        db1 = dp.sum(dim = 0)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        if len(hidden_dims) == 0:
          w1 = torch.normal(mean=0.0, std=weight_scale, size=(input_dim,num_classes), dtype=dtype)
          b1 = torch.zeros(num_classes, dtype=dtype)
          self.params["W1"] = w1.to(device)
          self.params["b1"] = b1.to(device)
          return

        w = torch.normal(mean=0.0, std=weight_scale, size=(input_dim,hidden_dims[0]), dtype=dtype)
        b = torch.zeros(hidden_dims[0], dtype=dtype)
        self.params["W1"] = w.clone().to(device)
        self.params["b1"] = b.clone().to(device)

        for i in range(1, len(hidden_dims)):
          w = torch.normal(mean=0.0, std=weight_scale, size=(hidden_dims[i-1],hidden_dims[i]), dtype=dtype)
          b = torch.zeros(hidden_dims[i], dtype=dtype)
          self.params["W%s"%(i+1)] = w.clone().to(device)
          self.params["b%s"%(i+1)] = b.clone().to(device)
        
        w = torch.normal(mean=0.0, std=weight_scale, size=(hidden_dims[len(hidden_dims)-1],num_classes), dtype=dtype)
        b = torch.zeros(num_classes, dtype=dtype)
        self.params["W%s"%(len(hidden_dims)+1)] = w.clone().to(device)
        self.params["b%s"%(len(hidden_dims)+1)] = b.clone().to(device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        X_reshaped = torch.reshape(X, (X.shape[0], -1))
        layers = {}
        linear = torch.mm(X_reshaped, self.params["W1"]) + self.params["b1"]
        layers["linear1"] = linear.clone()

        for i in range(2, self.num_layers+1):
          relu = linear.clone()
          relu[(relu < 0)] = 0
          layers["relu%s"%(i-1)] = relu.clone()
          linear = torch.mm(relu, self.params["W%s"%i]) + self.params["b%s"%i]
          layers["linear%s"%i] = linear.clone()
        ...
        for i in range(1, self.num_layers+1):
          L2 += (self.params["W%s"%i]**2).sum().item()
        loss += self.reg * L2
        
        dq = torch.mm(df, self.params["W%s"%self.num_layers].t())
        dwn = torch.mm(layers["relu%s"%(self.num_layers-1)].t(), df) + self.reg*2*self.params["W%s"%self.num_layers]
        dbn = df.sum(dim = 0)
        grads["W%s"%self.num_layers] = dwn
        grads["b%s"%self.num_layers] = dbn

        if self.num_layers > 1:
          for i in range(self.num_layers-1, 1, -1):
            dp = layers["linear%s"%i].clone()
            dp[(dp < 0)] = 0
            dp[(dp > 0)] = 1
            dp = torch.mul(dq, dp)

            dq = torch.mm(dp, self.params["W%s"%i].t())
            dwi = torch.mm(layers["relu%s"%(i-1)].t(), dp) + self.reg*2*self.params["W%s"%i]
            dbi = dp.sum(dim = 0)
            grads["W%s"%i] = dwi.clone()
            grads["b%s"%i] = dbi.clone()

          dp = layers["linear1"].clone()
          dp[(dp < 0)] = 0
          dp[(dp > 0)] = 1
          dp = torch.mul(dq, dp)
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict, num_epochs=80, device=device)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
