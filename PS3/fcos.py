"""
This module contains classes and functions that are used for FCOS,
a one-stage object detector. You have to implement the functions
here - walk through the notebooks and you will find instructions
on *when* to implement *what* in this module.
"""
...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def __init__(self, out_channels: int):
        ...
        ######################################################################
        # TODO: Initialize addi  tional Conv layers for FPN.                   #
        #                                                                    #
        ...
        # the input/output channels of these modules.                         #
            ...
            self.fpn_params["c3_1x1"] = nn.Conv2d(dummy_out_shapes[0][1][-3], out_channels, kernel_size=1)
            self.fpn_params["c4_1x1"] = nn.Conv2d(dummy_out_shapes[1][1][-3], out_channels, kernel_size=1)
            self.fpn_params["c5_1x1"] = nn.Conv2d(dummy_out_shapes[2][1][-3], out_channels, kernel_size=1)

            self.fpn_params["p3_output"] = nn.Sequential(
                nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1),
                nn.BatchNorm2d(out_channels),
                nn.ReLU(inplace=True),
                nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1),
            )
            self.fpn_params["p4_output"] = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)
            self.fpn_params["p5_output"] = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding=1)
            ######################################################################
            #                            END OF YOUR CODE                        #
            ######################################################################
    ...
    def forward(self, images: torch.Tensor):
        ...
        c3 = self.fpn_params["c3_1x1"](backbone_feats["c3"])
        c4 = self.fpn_params["c4_1x1"](backbone_feats["c4"])
        c5 = self.fpn_params["c5_1x1"](backbone_feats["c5"])

        p5 = self.fpn_params["p5_output"](c5)
        p4 = self.fpn_params["p4_output"](c4 + F.interpolate(p5, scale_factor=2, mode="nearest"))
        p3 = self.fpn_params["p3_output"](c3 + F.interpolate(p4, scale_factor=2, mode="nearest"))
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        ######################################################################
        h, w = feat_shape[-2:]
        # y_range = torch.arange(0, h * level_stride, level_stride, dtype=dtype, device=device)
        # x_range = torch.arange(0, w * level_stride, level_stride, dtype=dtype, device=device)
        # y_coords, x_coords = torch.meshgrid(y_range, x_range)
        x_range = torch.arange(0.5 * level_stride, w * level_stride, level_stride, dtype=dtype, device=device)
        y_range = torch.arange(0.5 * level_stride, h * level_stride, level_stride, dtype=dtype, device=device)
        x_coords, y_coords = torch.meshgrid(x_range, y_range)
...
def nms(boxes: torch.Tensor, scores: torch.Tensor, iou_threshold: float = 0.5):
    ...
    while sorted_indices.numel() > 0:
        ...
        boxes_remaining = boxes[sorted_indices[1:]]
        ...
        xx1 = boxes_remaining[:, 0]
        yy1 = boxes_remaining[:, 1]
        xx2 = boxes_remaining[:, 2]
        yy2 = boxes_remaining[:, 3]

        area_i = (x1 - x2) * (y1 - y2)
        # print("areai:  ", area_i)
        area_j = (xx1 - xx2) * (yy1 - yy2)
        # print("areaj:  ", area_j)
        w = torch.min(x2, xx2) - torch.max(x1, xx1)
        h = torch.min(y2, yy2) - torch.max(y1, yy1)
        inter = w * h
        # print("inter:  ", inter)
        mask = (w <= 0)
        inter[mask] = 0
        mask = (h <= 0)
        inter[mask] = 0
        ious = inter / (area_i + area_j - inter)
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        # for m in self.stem_cls.modules():
        for m in stem_cls:
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for level_name, features in feats_per_fpn_level.items():
            ...
            class_logits[level_name] = self.pred_cls(cls_features).permute(0, 3, 2, 1).reshape(features.size(0), features.size(2)*features.size(3), -1)
            boxreg_deltas[level_name] = self.pred_box(box_features).permute(0, 3, 2, 1).reshape(features.size(0), features.size(2)*features.size(3), -1)
            centerness_logits[level_name] = self.pred_ctr(box_features).permute(0, 3, 2, 1).reshape(features.size(0), features.size(2)*features.size(3), -1)
...
def fcos_get_deltas_from_locations(
    ...
    # deltas = gt_boxes[:, :4]
    # deltas[:, :2] -= locations
    # deltas[ :, [0, 2]] /= locations[:, 0].unsqueeze(1)
    # deltas[ :, [1, 3]] /= locations[:, 1].unsqueeze(1)
    # deltas = deltas.min(dim=1).values / stride
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    ##########################################################################
    mask = (deltas[:, 0] == -1)
    min_lr = deltas[:, ::2].min(dim=1).values
    max_lr = deltas[:, ::2].max(dim=1).values
    min_tb = deltas[:, 1::2].min(dim=1).values
    max_tb = deltas[:, 1::2].max(dim=1).values
    centerness_targets = ((min_lr * min_tb) / (max_lr * max_tb)).sqrt()
    centerness_targets[mask] = -1
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        # self.pred_net = FCOSPredictionNetwork(fpn_channels, num_classes)
        # Replace "PASS" statement with your code
        self.backbone = DetectorBackboneWithFPN(fpn_channels)
    ...
    def forward(
        ...
        locations_per_fpn_level = get_fpn_location_coords(
                fpn_feats_shapes, strides_per_fpn_level, images.dtype, images.device
        )
        ...
        for gt_box in gt_boxes:
          matched_gt_boxes.append(fcos_match_locations_to_gt(
            locations_per_fpn_level, strides_per_fpn_level, gt_box
          ))
        ...
        for matched_gt_box in matched_gt_boxes:
          deltas = {}
          for level_name, matched_box in matched_gt_box.items():
            deltas[level_name] = fcos_get_deltas_from_locations(
              locations_per_fpn_level[level_name], matched_box, strides_per_fpn_level[level_name]
            )
          matched_gt_deltas.append(deltas.copy())
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        classes = matched_gt_boxes[:, :, 4].clone()
        repeated_classes = torch.unsqueeze(classes, -1).repeat(1, 1, num_classes)
        classes = classes.clamp(min=0).long()

        gt_classes = torch.zeros_like(repeated_classes)
        gt_classes = gt_classes.scatter_(2, classes.unsqueeze(2), 1)

        mask = (repeated_classes == -1)
        gt_classes[mask] = 0
        ...
        gt_centerness = fcos_make_centerness_targets(matched_gt_deltas[0]).unsqueeze(0)
        for batch in range(1, matched_gt_deltas.shape[0]):
          gt_centerness = torch.cat((gt_centerness, fcos_make_centerness_targets(matched_gt_deltas[batch]).unsqueeze(0)), dim = 0)
        gt_centerness = gt_centerness.unsqueeze(-1)
        ...
        # # Replace "PASS" statement with your code
        is_background = (matched_gt_boxes[:, :, 4] < 0) #TODO
        loss_box[is_background] = 0
        loss_ctr[is_background] = 0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Compute the center coordinates and dimensions of the ground-truth boxes.
            stride = self.backbone.fpn_strides[level_name]
            x1 = level_locations[:, 0] - level_deltas[:, 0] * stride
            y1 = level_locations[:, 1] - level_deltas[:, 1] * stride
            x2 = level_locations[:, 0] + level_deltas[:, 2] * stride
            y2 = level_locations[:, 1] + level_deltas[:, 3] * stride

            level_pred_boxes = torch.stack((x1, y1, x2, y2), dim=1)

            # Step 4: Use `images` to get (height, width) for clipping.
            # Replace "PASS" statement with your code
            height, width = images.shape[2:]
            level_pred_boxes = level_pred_boxes.clamp(min=0)
