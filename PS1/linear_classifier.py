...
def predict_linear_classifier(W: torch.Tensor, X: torch.Tensor):
    ...
    y_pred = torch.max(y_pred, axis = 1)[1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_pred
...
def test_one_param_set(
    ...
    train_acc = (y_train == y_train_pred).double().mean().item()
    val_acc = (y_val == y_val_pred).double().mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(N):
        x = X[i]
        y_i = y[i]
        scores = torch.matmul(W.T,x)
        max_score = torch.max(scores)
        scores = scores - max_score
        correct_class_score = scores[y_i]
        sum_exp_score = torch.sum(torch.exp(scores))
        for j in range(C):
            if j == y_i:
                dW[:,j] += (torch.exp(correct_class_score)/sum_exp_score -1)* x
            else:
                dW[:,j] += (torch.exp(scores[j])/sum_exp_score)* x
        loss_i = - torch.log(torch.exp(correct_class_score)/sum_exp_score)
        loss += loss_i.item()
    loss = loss/N
    loss += reg * R
    dW = dW / N
...
def softmax_loss_vectorized(
    ...
    max_scores = torch.max(scores,axis = 1)[0]
    scores = scores - max_scores.unsqueeze(dim = 1)
    probs = torch.exp(scores)/torch.sum(torch.exp(scores), axis = 1).unsqueeze(dim = 1)
    correct_probs = probs[range(N),y].unsqueeze(dim = 1)
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [2e-03, 5e-02, 3e-03, 1e-04]
    regularization_strengths = [5e-04, 1e-05,3e-03]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
